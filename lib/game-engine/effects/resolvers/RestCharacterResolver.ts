/**
 * RestCharacterResolver.ts
 * 
 * Resolver for rest character effects. Changes target characters' state to RESTED.
 */

import { GameState, ZoneId, CardCategory, CardState } from '../../core/types';
import { EffectInstance } from '../types';
import { EffectResolver } from '../EffectResolver';
import { GameStateManager } from '../../core/GameState';

/**
 * Resolver for REST_CHARACTER effect type
 * 
 * Rests target characters by changing their state to RESTED.
 * Only affects characters in the character area.
 */
export class RestCharacterResolver implements EffectResolver {
  /**
   * Resolve a rest character effect
   * 
   * @param effect - The effect instance containing targets
   * @param state - The current game state
   * @returns Updated game state with characters rested
   */
  resolve(effect: EffectInstance, state: GameState): GameState {
    if (!effect.targets || effect.targets.length === 0) {
      // No targets - effect fizzles but doesn't error
      return state;
    }

    // Create a state manager to handle the rest operations
    let stateManager = new GameStateManager(state);

    // Rest each target character
    for (const target of effect.targets) {
      if (target.type !== 'CARD' || !target.cardId) {
        continue;
      }

      // Find the card in the state
      const card = stateManager.getCard(target.cardId);
      if (!card) {
        console.warn(`Rest target card not found: ${target.cardId}`);
        continue;
      }

      // Validate the card is a character
      if (card.definition.category !== CardCategory.CHARACTER) {
        console.warn(`Rest target is not a character: ${target.cardId}`);
        continue;
      }

      // Validate the card is in the character area
      if (card.zone !== ZoneId.CHARACTER_AREA) {
        console.warn(`Rest target is not in character area: ${target.cardId}`);
        continue;
      }

      // Check if already rested (no need to rest again)
      if (card.state === CardState.RESTED) {
        console.log(`Character ${target.cardId} is already rested`);
        continue;
      }

      // Rest the character by updating its state
      stateManager = stateManager.updateCard(card.id, {
        state: CardState.RESTED,
      });
    }

    // Get the updated state
    return stateManager.getState();
  }

  /**
   * Validate that a rest character effect can be resolved
   * 
   * @param effect - The effect instance to validate
   * @param state - The current game state
   * @returns True if the effect has valid parameters
   */
  canResolve(effect: EffectInstance, state: GameState): boolean {
    // Rest effects can always resolve (they just fizzle if no valid targets)
    return true;
  }
}
